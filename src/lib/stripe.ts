  ...
  try {
    ...
    const paymentLink = await stripe.paymentLinks.create({
      ...
      after_completion: {
        type: 'redirect',
        redirect: {
          url: `${process.env.NEXT_PUBLIC_APP_URL || process.env.BETTER_AUTH_URL}/obrigado/pagamento?session_id={CHECKOUT_SESSION_ID}&payment_link_id=${metadata.projectId || metadata.budgetId}`,
        },
      },
    });
