...
const categoryIcons: Record<string, React.ReactNode> = {
  ...
  TASK: <Briefcase className="h-3.5 w-3.5" />,
...
export default function AdminActivitiesPage() {
  ...
  useEffect(() => {
    fetchLogs();
  }, [currentPage, typeFilter, categoryFilter, levelFilter, dateRange]);
  ...
  const handleExportLogs = () => {
    const csvContent = [
      ["Data", "Tipo", "Categoria", "Nível", "Ação", "Usuário", "Entidade", "Descrição"],
      ...logs.map((log) => [
    ...
    link.href = url;
    link.download = `admin-activities-${new Date().toISOString().split("T")[0]}.csv`;
    link.click();
    URL.revokeObjectURL(url);

    toast({
      title: "Exportado!",
      description: "Atividades exportadas para CSV com sucesso.",
    });
  };
  ...
  const stats = {
    total: logs.length,
    creates: logs.filter((l) => l.type === "CREATE").length,
    updates: logs.filter((l) => l.type === "UPDATE").length,
    deletes: logs.filter((l) => l.type === "DELETE").length,
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              Rastreamento completo de todas as ações administrativas no sistema
            </p>
          </div>
                    ...
                    {logs.map((log) => (
                      <TableRow key={log.id}>
                        ...
                        <TableCell className="text-sm">
                          {log.user?.name || "Sistema"}
                          {log.user?.role === "admin" && (
                            <Badge className="ml-1 bg-blue-600 text-[10px]">Admin</Badge>
                          )}
                        </TableCell>
                        <TableCell className="text-sm">
                          {log.entityType ? (
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-semibold">Administrador</label>
                    <p className="text-sm">
                    ...
                    {selectedLog.user?.role === "admin" && (
                      <Badge className="mt-1 bg-blue-600 text-[10px]">Admin</Badge>
                    )}
                  </div>
                    ...
                    <div className="mt-2 space-y-2">
                      {selectedLog.changes.before && (
                        <div className="p-3 bg-red-50 border border-red-200 rounded text-xs">
                          ...
                          <pre className="mt-1 text-red-600 overflow-auto max-h-[150px]">
                      ...
                      {selectedLog.changes.after && (
                        <div className="p-3 bg-green-50 border border-green-200 rounded text-xs">
                          ...
                          <pre className="mt-1 text-green-600 overflow-auto max-h-[150px]">
