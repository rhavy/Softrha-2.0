 ...
 * 
 * Gestor aprova orçamento e envia proposta para o cliente.
 * Requer autenticação.
 * 
 * Envia e-mail com:
 * - Detalhes do orçamento
 * - Link de aprovação (tokenizado)
 */
export async function PUT(
  ...
  try {
    ...
    // Gerar/atualizar token de aprovação
        ...
        await resend.emails.send({
          ...
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              ...
              <p style="word-break: break-all; color: #2563eb; font-size: 12px;">${approvalUrl}</p>
    ...
    if (sendWhatsApp && budget.clientPhone) {
      ...
      const message = `Olá ${budget.clientName}! Segue sua proposta comercial:\n\nProjeto: ${budget.projectType}\nValor: R$ ${budget.finalValue?.toLocaleString("pt-BR")}\n\nAprovar: ${approvalUrl}`;
      whatsappUrl = `https://wa.me/55${phoneDigits}?text=${encodeURIComponent(message)}`;
    }
